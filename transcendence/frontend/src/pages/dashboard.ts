...
function renderTournamentMatch(m: TournamentMatch) {
  ...
  return `
    <div class="card-violet rounded-lg p-4 border ${finished ? 'border-gray-700' : 'border-purple-700'}">
      ...
      <div class="grid grid-cols-3 gap-4 items-center">
        <div class="text-right">
          <p class="font-semibold ${p1Win ? 'text-green-600' : 'text-gray-900'}">${m.player1.alias}</p>
        </div>
        <div class="text-center">
          <p class="text-2xl font-bold text-gray-900">${m.score.player1 || 0} : ${m.score.player2 || 0}</p>
        </div>
        <div class="text-left">
          <p class="font-semibold ${p2Win ? 'text-green-600' : 'text-gray-900'}">${m.player2.alias}</p>
        </div>
      </div>
