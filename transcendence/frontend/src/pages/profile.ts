...
export default function (root: HTMLElement, ctx?: { url?: URL }) {
  ...
  async function updateDisplayName() {
    ...
    const passwordInput = document.getElementById('confirm-password-display-name') as HTMLInputElement;
    const displayNameError = document.getElementById('display-name-error');
    const passwordError = document.getElementById('password-error-display-name');
    ...
    try {
      const token = getToken();
      const res = await fetch(`${GATEWAY_BASE}/user-service/users/${user.id}/update-display-name`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token || ''}`
