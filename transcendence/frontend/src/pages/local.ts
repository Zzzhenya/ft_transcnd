...
export default function (root: HTMLElement) {
  ...
  document.getElementById("createDemoGame")!.onclick = async () => {
    ...
    try {
      ...
      const res = await fetch(`${GATEWAY_BASE}/pong/demo`, {
        method: "POST",
        headers: {
  ...
  document.getElementById("connectGame")!.onclick = () => {
    ...
    ws = new WebSocket(`${WS_BASE}/ws/pong/game-ws/${gameId}${qs}`);

    ws.onopen = () => { output.textContent += "\nWebSocket connected!"; };
    ws.onmessage = (event) => { output.textContent += "\nReceived: " + event.data; };
