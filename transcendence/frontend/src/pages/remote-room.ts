...
//   totalScore: { player1: number; player2: number }; // <-- add this
...
//     gameState.centralWsUrl = websocketUrl;
//     gameState.connected = false;
//     (gameState as any).centralStartSent = false;
//     (gameState as any).centralGameEnded = false;
//   }
...
//     // prepare PLAYER_READY message
//     let numericUserId: number | null = null;
//     try {
//       const authUser = getAuth();
//       if (authUser && typeof authUser.id === 'number') numericUserId = authUser.id;
//       else {
//         const m = (gameState?.playerId || '').match(/^(\d+)_/);
//         numericUserId = (m && m[1]) ? parseInt(m[1], 10) : null;
//       }
//     } catch (e) { numericUserId = null; }
...
//         case "GAME_STARTED":
//         case "gameStarted":
//           if (gameState) (gameState as any).centralStartSent = false;
//           startGameUI(root, msg.gameState || msg.initialState || {});
//           break;
...
//             if (p1ready && p2ready) {
//               if (gameState && !(gameState as any).centralStartSent) {
//                 console.log('[Central WS] Both players ready — sending START_GAME to central engine');
//                 try {
//                   if (ws.readyState === WebSocket.OPEN) {
//                     ws.send(JSON.stringify({ type: 'START_GAME' }));
//                     (gameState as any).centralStartSent = true;
//                   }
//                 } catch (err) {
//                   console.warn('[Central WS] Failed to send START_GAME', err);
//                 }
...
//   const key = e.key;
//   const isUp = key === 'a' || key === 'A' || key === 'ArrowLeft';
//   const isDown = key === 'd' || key === 'D' || key === 'ArrowRight';
//   const now = Date.now();
...
//       try { gameState.ws.send(JSON.stringify({ type: 'MOVE_PADDLE', player: playerSlot, direction: 'up' })); } catch {}
//     }
...
//     if (now >= inputGateUntil) {
//       const playerSlot = gameState.playerNumber === 1 ? 'player1' : 'player2';
//       try { gameState.ws.send(JSON.stringify({ type: 'MOVE_PADDLE', player: playerSlot, direction: 'down' })); } catch {}
//     }
//     e.preventDefault(); e.stopPropagation();
//   }
// }
...
//   const key = e.key;
//   const isHandled = ['a','A','d','D','ArrowLeft','ArrowRight'].includes(key);
//   if (!isHandled) return;
...
//     const res = await fetch(`${API_BASE}/remote/match/finish`, {
//       method: 'POST',
//       credentials: 'include',
//       headers: { 'Content-Type': 'application/json' },
...
import { getAuth } from "@/app/auth";
import { getState } from "@/app/store";
import { WS_BASE, API_BASE } from "@/app/config";
import { createLocalScene } from "@/renderers/babylon/local-scene";


/**
 * updateStatus - small UI status box update helper
 */
function updateStatus(root: HTMLElement, message: string, type: 'info' | 'success' | 'error') {
  const box = root.querySelector('#statusBox') as HTMLElement | null;
  const text = root.querySelector('#statusText') as HTMLElement | null;
  if (!box || !text) return;
  box.className = `rounded border-2 p-4 ${
    type === 'success'
...
/**
 * updateDiagnostics - compact diagnostics update helper (ping, role)
 * Keep it minimal and safe to call early.
 */
function updateDiagnostics(root: HTMLElement) {
  try {
    const pingEl = root.querySelector('#pingDisplay') as HTMLElement | null;
    ...
    // swallow, diagnostics should never throw
    console.debug('updateDiagnostics error', err);
  }
}
...
export default function (root: HTMLElement, ctxObj: { params?: { roomId?: string }; url: URL }) {
  ...
  gameState = {
    ...
    gameStarted: false,
    score: { player1: 0, player2: 0 },
    totalScore: { player1: 0, player2: 0 }, // <-- ensure initialized
    players: [],
    centralGameId: null,
    centralWsUrl: null,
    centralStartSent: false,
    ...
    <section class="retro-wait py-6 md:py-8 space-y-6 max-w-6xl mx-auto px-4">
      <div class="crt-scan vignette bezel rounded-2xl p-5 md:p-6 border border-purple-500/30">
        ...
        <div id="waitingRoom" class="mt-6 rounded-xl p-6 space-y-5 bezel border border-purple-500/30 bg-black/30">
          ...
          <h2 class="text-xl font-black neon mt-2">PLAYERS <span class="blink"></span></h2>
          <div class="space-y-2"><div id="playersList" class="space-y-2"></div></div>
        ...
        <div id="gameContainer" class="hidden mt-6 relative">
          ...
          <div class="mx-auto" style="width:1000px;max-width:100%;"><canvas id="gameCanvas" tabindex="0" width="1000" height="600" class="bg-black block w-full h-auto"></canvas></div>
        </div>
      </div>
    </section>
  `;
...
function setupRoomEventListeners(root: HTMLElement) {
  ...
  root.querySelector("#copyCode")?.addEventListener("click", () => {
    if (gameState?.roomId) navigator.clipboard.writeText(gameState.roomId).then(() => updateStatus(root, "✅ Room code copied!", "success"));
  });

  root.querySelector("#readyBtn")?.addEventListener("click", async () => {
    const btn = document.querySelector<HTMLButtonElement>("#readyBtn")!;
    if (!gameState?.connected) { updateStatus(root, "⚠️ Not connected", "error"); return; }
    if (!gameState.serverIdReady) { updateStatus(root, "Syncing with server…", "info"); setTimeout(() => refreshReadyButtonState(root), 100); return; }
    ...
    try {
      ...
      btn.disabled = true;
      btn.textContent = "⏳ Waiting for opponent...";
    } catch (e) { console.error("Failed to send ready", e); }
  });
...
async function httpFallbackReady(root: HTMLElement, uname: string) {
  try {
    ...
    if (!res.ok) { console.warn("HTTP fallback ready failed", res.status); updateStatus(root, "⚠️ Ready fallback failed", "error"); return; }
    updateStatus(root, "✅ Ready (fallback)", "success");
  } catch (err) { console.error("HTTP fallback error", err); updateStatus(root, "❌ Ready fallback error", "error"); }
}

/* ---------- Lobby WS ---------- */
function connectToRoom(root: HTMLElement, roomId: string, playerId: string, username: string) {
  try { if (gameState?.ws) { try { gameState.ws.onopen = gameState.ws.onmessage = gameState.ws.onclose = gameState.ws.onerror = null as any; } catch {} try { gameState.ws.close(); } catch {} } } catch {}
  ...
  ws.onmessage = async (event) => {
    ...
    try {
      ...
      const message = JSON.parse(data);
      handleServerMessage(root, message);
    } catch (err) { console.error("Error parsing lobby message:", err); }
  };

  ws.onerror = (e) => { console.error("Lobby WS error", e); updateStatus(root, "❌ Connection error", "error"); };
  ws.onclose = () => {
    if (!gameState) return;
    gameState.connected = false;
    if (!isTransitioningToCentral) {
      updateStatus(root, "⚠️ Disconnected", "error");
      refreshReadyButtonState(root);
    } else console.log("[remote-room] lobby WS closed due to central transition");
  };
}
...
function handleEngineStateUpdate(root: HTMLElement, msg: any, centralGameId?: number) {
  ...
  // Update client-side totalScore if provided by engine
  if (state.totalScore && gameState) {
    // normalize numbers
    gameState.totalScore = {
      player1: Number(state.totalScore.player1 || 0),
      player2: Number(state.totalScore.player2 || 0),
    };
  }

  // Detect tournament finish...
  if (state.tournament && state.tournament.gameStatus === "gameEnd") {
    if (gameState && !(gameState as any).centralGameEnded) {
      (gameState as any).centralGameEnded = true;
      const winner = state.tournament.winner;
      const finalScore = state.score || { player1: 0, player2: 0 };
      endGame(root, { winner, finalScore, totalScore: state.totalScore, gameId: centralGameId });
    }
  }
...
function connectToCentralGame(root: HTMLElement, websocketUrl: string, centralGameId: number) {
  ...
  ws.onopen = () => {
    ...
    console.log('[Central WS] sending PLAYER_READY ', readyMsg);
    try { if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(readyMsg)); } catch (err) { console.warn('[Central WS] send PLAYER_READY failed', err); }
  };
  ...
  ws.onmessage = async (ev) => {
    try {
      ...
      switch (String(msg.type)) {
        ...
        case "gameStarted":
          if (gameState) gameState.centralStartSent = false;
          // engine might include gameState with totalScore
          if (msg.gameState?.totalScore) {
            if (gameState) gameState.totalScore = { player1: Number(msg.gameState.totalScore.player1 || 0), player2: Number(msg.gameState.totalScore.player2 || 0) };
          }
          startGameUI(root, msg.gameState || msg.initialState || {});
          break;
        ...
        case "GAME_ENDED": {
          ...
          const normalized: any = {
            winner: msg.winner ?? msg.winnerPlayerNumber ?? msg.winnerPlayer,
            finalScore: msg.finalScores ?? msg.finalScore ?? msg.score,
            totalScore: msg.totalScore ?? null,
            gameId: msg.gameId ?? centralGameId
          };
          endGame(root, normalized);
          break;
        ...
        case "PLAYER_READY_UPDATE":
          try {
            ...
            if (p1ready && p2ready) {
              if (gameState && !gameState.centralStartSent) {
                try { if (ws.readyState === WebSocket.OPEN) { ws.send(JSON.stringify({ type: "START_GAME" })); if (gameState) gameState.centralStartSent = true; } }
                catch (err) { console.warn('[Central WS] Failed to send START_GAME', err); }
              }
            }
          } catch (e) { console.error('[Central WS] Error handling PLAYER_READY_UPDATE', e); }
          break;

        case "pong":
          try { const now = Date.now(); const rtt = msg.ts ? now - msg.ts : lastPingTs ? now - lastPingTs : null; const el = root.querySelector("#pingDisplay"); if (el && rtt !== null) el.textContent = `Ping: ${rtt} ms`; } catch (e) {}
          break;

        default:
          console.debug("[Central WS] unhandled", msg);
      }
    } catch (err) { console.error("[Central WS] parse error", err); }
  };
  ...
  ws.onerror = (e) => { console.error("[Central WS] error", e); updateStatus(root, "❌ Central WS error", "error"); };
}
...
function handleServerMessage(root: HTMLElement, message: any) {
  ...
  switch (message.type) {
    case "pong":
      try { const now = Date.now(); const rtt = lastPingTs && message.ts ? now - message.ts : lastPingTs ? now - lastPingTs : null; const el = root.querySelector("#pingDisplay"); if (el) el.textContent = rtt !== null ? `Ping: ${rtt} ms` : "Ping: -- ms"; } catch {}
      break;

    case "init":
      if (message.playerId && gameState?.playerId !== message.playerId) gameState!.playerId = message.playerId;
      if (gameState) { gameState.serverIdReady = !!message.playerId; gameState.playerNumber = message.playerNumber; }
      updateStatus(root, `You are Player ${message.playerNumber}`, "success");

      {
        ...
        const myPlayerId = gameState?.playerId;
        const hasSelf = players.some((p: any) => p.playerId === myPlayerId);
        if (!hasSelf && gameState) players.push({ playerId: myPlayerId || '', playerNumber: message.playerNumber, username: "You", ready: false });
        if (gameState) gameState.players = players;
      }
      updatePlayersList(root);
      setTimeout(() => refreshReadyButtonState(root), 0);
    ...
    case "startGame":
      console.log('[remote-room] startGame received from lobby:', message);
      if (gameState) { gameState.centralGameId = message.gameId; gameState.centralWsUrl = message.websocketUrl; }

      // Ensure playerNumber is set
      if (!gameState || (gameState.playerNumber !== 1 && gameState.playerNumber !== 2)) {
        console.warn('[remote-room] missing playerNumber on startGame; delaying central connect');
        setTimeout(() => {
          if (gameState && (gameState.playerNumber === 1 || gameState.playerNumber === 2)) {
    ...
    case "playerDisconnected":
      if (isTransitioningToCentral) { console.log("[remote-room] playerDisconnected during central transition – ignoring"); break; }
      updateStatus(root, "⚠️ Opponent disconnected", "error");
      setTimeout(() => { if (confirm("Opponent left. Return to remote lobby?")) navigate("/remote"); }, 2000);
      break;
...
function refreshReadyButtonState(root: HTMLElement) {
  ...
  if (wsOpen && !gs.serverIdReady && (!gs.players || gs.players.length === 0)) {
    updateStatus(root, '🔄 Syncing with server…', 'info');
    setTimeout(() => refreshReadyButtonState(root), 150);
    btn.disabled = true; btn.textContent = '⏳ Waiting for opponent...'; return;
  }
  const p1 = gs.players.find(p => p.playerNumber === 1);
  const p2 = gs.players.find(p => p.playerNumber === 2);
  const twoRolesAssigned = !!p1 && !!p2;
...
function startGameUI(root: HTMLElement, initialState: any) {
  ...
  // Seed scores/totalScore if engine provided them
  if (initialState) {
    if (initialState.score) gameState.score = { player1: Number(initialState.score.player1 || 0), player2: Number(initialState.score.player2 || 0) };
    if (initialState.totalScore) gameState.totalScore = { player1: Number(initialState.totalScore.player1 || 0), player2: Number(initialState.totalScore.player2 || 0) };
  }
  ...
  try {
    ...
    const tick = () => {
      if (!sceneController) return;
      if (lastRenderState) { try { sceneController.update(lastRenderState); } catch {} }
      rafId = requestAnimationFrame(tick);
    };
    rafId = requestAnimationFrame(tick);
  } catch (e) {
...
function updateGameState(root: HTMLElement, state: any) {
  ...
  if (state.score) gameState.score = { player1: Number(state.score.player1 || 0), player2: Number(state.score.player2 || 0) };
  if (state.totalScore) gameState.totalScore = { player1: Number(state.totalScore.player1 || 0), player2: Number(state.totalScore.player2 || 0) };
  ...
  if (sceneController && typeof sceneController.update === 'function') {
    try {
      ...
      const renderState = {
        ...
        paddles: { player1: effectiveP1, player2: effectiveP2 },
        score: { player1: state.score?.player1 ?? 0, player2: state.score?.player2 ?? 0 },
        totalScore: { player1: gameState.totalScore.player1, player2: gameState.totalScore.player2 },
        match: { roundsWon: { player1: 0, player2: 0 }, winner: null, currentRound: 1 },
        gameStatus: 'playing'
      };
      lastRenderState = renderState;
...
function drawGame(state: any) {
  ...
  ctx.strokeStyle = '#333'; ctx.setLineDash([15,15]); ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(canvas.width/2,0); ctx.lineTo(canvas.width/2, canvas.height); ctx.stroke(); ctx.setLineDash([]);
  const paddleWidth = 15, paddleHeight = 100, paddleOffset = 40;
  ctx.fillStyle = '#3b82f6'; const p1Y = canvas.height/2 - state.paddles.player1; ctx.fillRect(paddleOffset, p1Y - paddleHeight/2, paddleWidth, paddleHeight);
  ctx.fillStyle = '#ef4444'; const p2Y = canvas.height/2 - state.paddles.player2; ctx.fillRect(canvas.width - paddleOffset - paddleWidth, p2Y - paddleHeight/2, paddleWidth, paddleHeight);
  ctx.fillStyle = '#fff'; ctx.shadowBlur = 20; ctx.shadowColor = '#fff'; ctx.beginPath(); ctx.arc(canvas.width/2 + state.ball.x, canvas.height/2 - state.ball.y, 12, 0, Math.PI*2); ctx.fill(); ctx.shadowBlur = 0;
}
...
function handleKeyUp(e: KeyboardEvent) {
  ...
  const isHandled = ['a','A','d','D','ArrowLeft','ArrowRight'].includes(key);
  if (!isHandled) return;
  const wasUp = keysPressed.has('up'), wasDown = keysPressed.has('down');
...
function endGame(root: HTMLElement, data: any) {
  ...
  // Ensure data.totalScore/finalScore normalized
  const payloadScore = data.totalScore ?? data.finalScore ?? data.finalScores ?? data.score ?? null;
  if (payloadScore) {
    gameState.totalScore = { player1: Number(payloadScore.player1 || 0), player2: Number(payloadScore.player2 || 0) };
  }

  reportRemoteMatchFinish({ ...data, totalScore: gameState.totalScore }).catch(err => console.error("[Remote] report failed", err));

  const display = root.querySelector('#countdownDisplay') as HTMLElement | null;
  if (display) {
    display.textContent = isYouWinner ? '🏆 YOU WON! 🏆' : `${winner} Won!`;
    display.style.display = 'block';
    display.className = 'absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 text-5xl font-bold ' + (isYouWinner ? 'text-green-400' : 'text-yellow-400');
  }

  updateStatus(root, isYouWinner ? '🎉🏆 YOU WON! 🏆🎉' : `${winner} won!`, isYouWinner ? 'success' : 'info');
  setTimeout(() => navigate('/remote'), 3000);
}
...
async function reportRemoteMatchFinish(data: any) {
  try {
    ...
    // Prefer totals from engine payload, then client totalScore, then payload finalScore, then per-round score
    const totalsFromPayload = data.totalScore ?? null;
    const finalScoreFromPayload = data.finalScore ?? data.finalScores ?? null;

    const totals = totalsFromPayload
      ? { player1: Number(totalsFromPayload.player1 || 0), player2: Number(totalsFromPayload.player2 || 0) }
      : (gameState.totalScore ? { player1: Number(gameState.totalScore.player1 || 0), player2: Number(gameState.totalScore.player2 || 0) } : null);

    let p1Total: number, p2Total: number;
    if (totals) { p1Total = totals.player1; p2Total = totals.player2; }
    else if (finalScoreFromPayload) { p1Total = Number(finalScoreFromPayload.player1 || 0); p2Total = Number(finalScoreFromPayload.player2 || 0); }
    else { p1Total = Number(gameState.score.player1 || 0); p2Total = Number(gameState.score.player2 || 0); }

    let winnerStr: 'player1'|'player2';
    if (data.winner === 1 || data.winner === 'player1') winnerStr = 'player1';
    else if (data.winner === 2 || data.winner === 'player2') winnerStr = 'player2';
    else {
      if (p1Total > p2Total) winnerStr = 'player1';
      else if (p2Total > p1Total) winnerStr = 'player2';
      else { console.warn('[Remote] Unknown winner and totals equal'); return; }
    }

    const gameId = data.gameId ?? gameState.centralGameId ?? null;
    if (!gameId) { console.warn('[Remote] No gameId to report'); return; }

    const payload = { gameId, winner: winnerStr, finalScore: { player1: p1Total, player2: p2Total } };

    // Note: ensure API_BASE resolves to /api/game or whichever prefix your gateway expects.
    const res = await fetch(`${API_BASE}/remote/match/finish`, {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) {
      console.warn('[Remote] finish endpoint failed', res.status, await res.text().catch(()=>''));
      return;
    }
    const json = await res.json().catch(()=>null);
    console.log('[Remote] Remote match recorded:', json);
  } catch (err) {
    console.error('[Remote] reportRemoteMatchFinish error', err);
  }
}
