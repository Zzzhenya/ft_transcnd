...
export default function (
  root: HTMLElement,
  ctxObj: { params?: { roomId?: string }; url: URL }
) {
  ...
  const playerId = `${user?.id ?? "guest"}_${Date.now()}_${Math.random()
    .toString(36)
    .substring(2)}`;
  ...
  if (didMountRemoteRoom || isConnectingRemoteRoom) {
    console.warn(
      "⚠️ Remote room already mounted/connecting, skipping duplicate setup"
    );
  } else {
    ...
    <section class="retro-wait py-6 md:py-8 space-y-6 max-w-6xl mx-auto px-4">
      <div class="crt-scan vignette bezel rounded-2xl p-5 md:p-6 border border-purple-500/30">
        <div class="flex items-center justify-between">
          ...
          <button id="leaveRoom" class="btn-retro px-4 py-2 rounded-lg text-white">
            ⏎ LEAVE
          </button>
        </div>
  ...
  // Event listeners for invites/leave notifications
  const onInviteDeclined = (e: Event) => {
    if (!gameState) return;
    const detail: any = (e as CustomEvent).detail || {};
    updateStatus(
      root,
      `❌ ${detail?.from || "Player"} declined your invitation`,
      "error"
    );
  ...
  const onPlayerLeft = (e: Event) => {
    ...
    updateStatus(
      root,
      `👋 ${detail?.from || "Player"} left the waiting room`,
      "error"
    );
    setTimeout(() => navigate("/remote"), 1500);
  };
  ...
  return () => {
    ...
    window.removeEventListener(
      "invite:declined",
      onInviteDeclined as EventListener
    );
  ...
  root.querySelector("#readyBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        const uname =
          getAuth()?.name || getState().session.alias || "Anonymous";
      ...
      if (!pingTimer && gameState?.ws) {
        pingTimer = window.setInterval(() => {
          try {
            lastPingTs = Date.now();
            gameState?.ws?.send(
              JSON.stringify({ type: "ping", ts: lastPingTs })
            );
          } catch {}
        }, 3000);
      }
...
async function httpFallbackReady(root: HTMLElement, uname: string) {
  try {
    if (!gameState) return;
    const { roomId, playerId, wsConnectedRoomId, playerNumber } =
      gameState as any;
    const rid = wsConnectedRoomId || roomId;

    const res = await fetch(
      `/api/game/rooms/${encodeURIComponent(
        rid
      )}/players/${encodeURIComponent(playerId)}/ready`,
      {
        method: "POST",
        credentials: "include",
...
function connectToRoom(
  ...
  const wsUrl = `${WS_BASE}/remote?roomId=${roomId}&playerId=${playerId}&username=${encodeURIComponent(
    username
  )}`;
  ...
  if (gameState) {
    gameState.ws = ws;
    (gameState as any).connectionId = thisConnId;
...
function connectToCentralGame(
  root: HTMLElement,
  websocketUrl: string,
  centralGameId: number
) {
  // close any existing ws cleanly
  try {
    if (gameState?.ws) {
      try {
        gameState.ws.onopen =
  ...
  ws.onopen = () => {
    ...
    // AUTO: tell central engine we're ready – use slot from lobby
    const slot =
    ...
    if (slot) {
      ...
      try {
        ...
        console.warn("[Central WS] send PLAYER_READY failed", err);
      }
    } else {
      console.warn(
        "[Central WS] no playerNumber assigned, not sending PLAYER_READY"
      );
    }
  };
      ...
      switch (String(msg.type)) {
        ...
        case "GAME_ENDED": {
          ...
          const normalized: any = {
            winner:
              msg.winner ??
              msg.winnerPlayerNumber ??
              msg.winnerPlayer,
            finalScore:
              msg.finalScores ?? msg.finalScore ?? msg.score,
        ...
        case "PLAYER_READY_UPDATE":
          try {
            console.log("[Central WS] PLAYER_READY_UPDATE", msg);

            const playersReady =
              msg.playersReady ?? msg.state?.playersReady ?? null;
            ...
            if (p1ready && p2ready) {
              if (gameState && !(gameState as any).centralStartSent) {
                console.log(
                  "[Central WS] Both players ready — sending START_GAME to central engine"
                );
                try {
                  if (ws.readyState === WebSocket.OPEN) {
                  ...
                  console.warn(
                    "[Central WS] Failed to send START_GAME",
                    err
                  );
                }
              } else {
                console.log(
                  "[Central WS] START_GAME already sent or gameState missing"
                );
              }
            } else {
              console.log(
                "[Central WS] PLAYER_READY_UPDATE shows not all ready",
                { p1ready, p2ready }
              );
            }
          } catch (e) {
            console.error(
              "[Central WS] Error handling PLAYER_READY_UPDATE",
              e
            );
          }
          break;
...
function handleEngineStateUpdate(root: HTMLElement, msg: any, centralGameId?: number) {
  ...
  if (state.tournament && state.tournament.gameStatus === "gameEnd") {
    // Avoid calling endGame multiple times on repeated state updates
    if (gameState && !(gameState as any).centralGameEnded) {
      (gameState as any).centralGameEnded = true;

      const winner = state.tournament.winner; // "player1" / "player2" or 1 / 2 depending on backend
      const finalScore = state.score || { player1: 0, player2: 0 };

      endGame(root, {
        winner,         // endGame can handle "player1"/"player2" or 1/2
        finalScore,     // { player1, player2 }
        gameId: centralGameId
      });
    }
  }
...
function handleServerMessage(root: HTMLElement, message: any) {
  ...
  switch (message.type) {
    ...
    case "init":
      ...
      updateStatus(
        root,
        `You are Player ${message.playerNumber}`,
        "success"
      );
      {
        if (
          message.roomInfo &&
          typeof message.roomInfo.roomId === "string"
        ) {
          gameState.wsConnectedRoomId = message.roomInfo.roomId;
        }
        const serverPlayers = Array.isArray(
          message.roomInfo?.players
        )
          ? message.roomInfo.players
          : [];
        ...
        const players = Array.from(dedupMap.values());
        const myPlayerId = gameState.playerId;
        const hasSelf = players.some(
    ...
    case "startGame":
      console.log(
        "[remote-room] startGame received from lobby:",
        message
      );
      if (gameState) {
      ...
      if (message.websocketUrl && message.gameId) {
        console.log(
          "[remote-room] connecting to central WS:",
          message.websocketUrl
        );
        connectToCentralGame(root, message.websocketUrl, message.gameId);
      } else {
        console.warn(
          "[remote-room] startGame missing websocketUrl or gameId:",
          message
        );
      }
      break;
    ...
    case "playerDisconnected":
      if (isTransitioningToCentral) {
        console.log(
          "[remote-room] playerDisconnected during central transition – ignoring"
        );
        break;
      }
      ...
      setTimeout(() => {
        if (
          confirm("Opponent left. Return to remote lobby?")
        )
          navigate("/remote");
      }, 2000);
      break;
  ...
  container.innerHTML = players
    ...
    <div class="player-card">
      <div class="player-badge ${
        p.playerNumber === 1 ? "bg-blue-600" : "bg-red-600"
      }">P${p.playerNumber}</div>
      <div class="flex-1">
        <div class="font-black text-indigo-100">${p.username}</div>
        ${
          p.playerId === gameState?.playerId
            ? '<div class="chip text-[11px] text-indigo-200/90">YOU</div>'
            : ""
        }
      </div>
      ${
        p.ready
          ? '<div class="status-ready text-sm"><span class="status-dot" style="background:#4ade80;"></span>READY</div>'
...
function refreshReadyButtonState(root: HTMLElement) {
  ...
  const me =
    gs.players.find((p) => p.playerId === gs.playerId) || null;
...
function startGameUI(root: HTMLElement, initialState: any) {
  ...
  try {
    ...
    const tick = () => {
      ...
      if (lastRenderState) {
        try {
          sceneController.update(lastRenderState);
        } catch {}
      }
...
function updateGameState(root: HTMLElement, state: any) {
  ...
  if (sceneController && typeof sceneController.update === "function") {
    try {
      ...
      try {
        sceneController.update(renderState as any);
      } catch {}
      return;
    } catch (e) {
...
function drawGame(state: any) {
  ...
  ctx.beginPath();
  ctx.arc(
    canvas.width / 2 + state.ball.x,
    canvas.height / 2 - state.ball.y,
    12,
    0,
    Math.PI * 2
  );
...
function handleKeyDown(e: KeyboardEvent) {
  ...
  const sendMove = (dir: "up" | "down" | "stop") => {
    const playerSlot =
      gameState!.playerNumber === 1 ? "player1" : "player2";
    try {
      gameState!.ws!.send(
        JSON.stringify({ type: "MOVE_PADDLE", player: playerSlot, direction: dir })
      );
    } catch (err) {
      console.warn("Failed to send MOVE_PADDLE", err);
    }
  };
...
function updateScoreDisplay(root: HTMLElement) {
  if (!gameState) return;
  root.querySelector("#scoreP1")!.textContent = String(
    gameState.score.player1
  );
  root.querySelector("#scoreP2")!.textContent = String(
...
function endGame(root: HTMLElement, data: any) {
  ...
  reportRemoteMatchFinish(data).catch((err) =>
  ...
  if (display) {
    display.textContent = isYouWinner
      ? "🏆 YOU WON! 🏆"
      : `Player ${winner} Won!`;
    display.style.display = "block";
    display.className =
      "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 text-5xl font-bold " +
...
async function reportRemoteMatchFinish(data: any) {
  try {
    if (!gameState) return;
    const finalScore =
      data.finalScore || data.finalScores || gameState.score;
    ...
    if (data.winner === 1 || data.winner === "player1")
      winnerStr = "player1";
    else if (data.winner === 2 || data.winner === "player2")
      winnerStr = "player2";
    else {
      console.warn("Unknown winner", data.winner);
      return;
    }

    const gameId = data.gameId ?? gameState.centralGameId ?? null;
    if (!gameId) {
      console.warn("No gameId to report");
      return;
    }
    ...
    const res = await fetch("/api/game/remote/match/finish", {
      ...
      body: JSON.stringify({
        gameId,
        winner: winnerStr,
        finalScore: { player1: p1, player2: p2 },
      }),
    });

    if (!res.ok) {
      console.warn("finish endpoint failed", res.status);
      return;
    }
...
function updateDiagnostics(root: HTMLElement) {
  try {
    const pingEl = root.querySelector(
      "#pingDisplay"
    ) as HTMLElement | null;
    if (pingEl) {
      if (
    ...
    const roleEl = root.querySelector(
      "#yourRole"
    ) as HTMLElement | null;
    if (roleEl && gameState) {
      roleEl.textContent = gameState.playerNumber
