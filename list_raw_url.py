...
for item in data.get("tree", []):
    if item["type"] == "blob":  # only files
        ...
        if not item["path"].startswith("legacy/") and not item["path"].startswith("frontend_jason/") and not item["path"].startswith(".github/") and not item["path"].startswith("eval") and not item["path"].startswith("script") and not item["path"].startswith("shared") :
